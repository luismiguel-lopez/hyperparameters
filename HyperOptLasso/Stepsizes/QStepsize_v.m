classdef QStepsize_v < StepsizePolicy
    ...
    % The difference between this and QStepsize is that this one keeps the
    % gradients in the same direction as the UFTML one, and tries to
    % control a scale factor that is applied to the gradient. If QStepsize
    % is run in multiple dimensions, each component is scaled by a
    % different scale factor. No idea which is better yet.
    ...
    methods
        function v_eta_out = update_stepsize(obj, v_g, ~)
            ...
            obj.v_eta = v_eta_ftml*obj.eta_now./obj.eta_0;
