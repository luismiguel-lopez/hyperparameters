classdef FranceschiRecursiveElasticNet
...
methods
    ...
    function [v_w_t, lambda_rho_next, loss_t, v_wf_next, m_Phi_next, v_r_next, m_c_next] = update(obj, ...
        lambda_rho_t, v_x_t, y_t, v_wf_t, m_Phi_t, v_r_t, m_c_t)
        ...
        lambda_t = lambda_rho_t(1);
        rho_t    = lambda_rho_t(2);
        ...
        m_c_next = m_c_t - alpha*m_Phi_t*m_c_t - alpha*rho_t*m_c_t - alpha*[v_z_t, v_w_t];
        grad_t = -prediction_error* v_x_t'*m_c_t;
        lambda_rho_next = max(0, lambda_rho_t - beta*grad_t');
        v_wf_next = v_w_t - alpha*(m_Phi_next*v_w_t + rho_t*v_w_t - v_r_next);
    end
end
