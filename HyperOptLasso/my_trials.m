...
stem(v_s_plus);
hold on
stem(v_w_j(vb_support)-v_w_star(vb_support))

figure(103); clf
stem(m_Psi*v_w_star(vb_support));
hold on
stem(m_Psi*v_w_j(vb_support));

figure(105); clf
stem(m_Psi*(v_w_star(vb_support)-v_w_j(vb_support)));

v_w_j2 = v_w_star; v_w_j2(vb_support) = v_w_j2(vb_support) + v_s_plus;

figure(104); clf;
title 'subdifferential violations'
stem(max(0, abs(m_F_j*v_w_j    - v_r_j)-1)); hold on
stem(max(0, abs(m_F_j*v_w_j2   - v_r_j)-1));
stem(max(0, abs(m_F_j*v_w_star - v_r_j)-1));
