classdef FranceschiRecursiveAdaptiveLasso
...
methods
    ...
    function [v_w_t, lambda_next, loss_t, v_wf_next, m_Phi_next, v_r_next, v_c_next] = update(obj, ...
        lambda_t, v_x_t, y_t, v_wf_t, m_Phi_t, v_r_t, v_c_t, v_weights)
        ...
        v_w_t = obj.soft_thresholding(v_wf_t, alpha*lambda_t.*v_weights);

        prediction_error = y_t - v_x_t'*v_w_t;

        loss_t = prediction_error.^2;
        v_z_t = max(-1, min(1, v_wf_t./(alpha*lambda_t))); %soft
        v_c_next = v_c_t - alpha*m_Phi_t*v_c_t - alpha*v_z_t.*v_weights;
